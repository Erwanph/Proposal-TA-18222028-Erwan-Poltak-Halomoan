...
Prediksi data \textit{deret waktu} (\textit{time series forecasting}) bertujuan untuk memodelkan perilaku dinamis suatu variabel berdasarkan urutan nilai historisnya untuk memproyeksikan nilai masa depan \autocite{box1970time}. Dalam konteks ekonomi makro, tantangan utama prediksi ini terletak pada karakteristik data yang sering kali mengandung autokorelasi tinggi, volatilitas yang berubah-ubah (\textit{heteroskedasticity}), dan interaksi non-linear antarvariabel yang kompleks. Secara historis, pendekatan ini didominasi oleh model ekonometrika linear. Namun, seiring dengan ledakan volume data (\textit{Big Data}) dan kebutuhan akurasi yang lebih tinggi, terjadi pergeseran paradigma menuju penggunaan model \textit{Machine Learning} dan \textit{Deep Learning}. Varian menegaskan bahwa teknik \textit{Machine Learning} menawarkan alat baru bagi para ekonom untuk menangkap pola non-linear yang sering kali terlewatkan oleh model regresi standar, menjadikannya komplemen yang penting bagi metode tradisional.

\subsection{Model Tradisional (Ekonometrika)}
Pada subbab ini dibahas model ekonometrika tradisional yang menjadi landasan awal dalam prediksi ekonomi karena kemampuannya dalam menjelaskan hubungan yang struktural antarvariabel secara transparan, meskipun kinerjanya yang terbatas pada asumsi linearitas yang kuat (ini tolong kasih sitasi).

\subsubsection{Autoregressive Integrated Moving Average (ARIMA)}
ARIMA adalah model statistik univariat yang banyak digunakan untuk \textit{time series forecasting}. Model ini dikembangkan oleh Box dan Jenkins pada tahun 1970 dan menjadi standar dalam prediksi ekonomi \autocite{box1970time}. ARIMA terdiri dari tiga komponen utama:
\begin{enumerate}
\item \textbf{Autoregressive (AR)}\\
...
Secara matematis, model ARIMA(p,d,q) dapat dituliskan seperti pada Persamaan \ref{eq:arima}:
...
Dengan $\phi(B)$ adalah polinomial AR, $B$ adalah \textit{backshift operator}, $\theta(B)$ adalah operator polinomial MA, dan $\epsilon_t$ adalah \textit{white noise error} \autocite{box2015time}. Keunggulan utama ARIMA adalah efisiensinya pada data linear jangka pendek, tetapi model ini sering gagal menangkap volatilitas tinggi atau perubahan struktural mendadak (\textit{structural breaks}) yang umum terjadi dalam data ekonomi modern.

\subsubsection{Vector Autoregression (VAR)}
VAR adalah perluasan multivariat dari model \textit{autoregressive} yang dikembangkan oleh Sims \autocite{sims1980macroeconomics}. VAR memodelkan sistem variabel endogen, yaitu setiap variabel dijelaskan oleh nilai \textit{lag}-nya sendiri dan nilai \textit{lag} variabel lain dalam sistem. Model VAR(p) untuk $k$ variabel dapat dituliskan sebagai:
...
Dengan $Y_t$ adalah vektor $(K \times 1)$ variabel endogen, $c$ adalah vektor konstanta, $A_i$ adalah matriks koefisien, dan $u_t$ adalah vektor \textit{white noise} \autocite{lutkepohl2011vector}.

\subsubsection{Vector Error Correction Model (VECM)}
VECM adalah model VAR yang telah dimodifikasi untuk menangkap hubungan kointegrasi antar variabel \textit{non-stationary}. Model ini dikembangkan dari konsep \textit{cointegration} oleh Engle dan Granger dan merupakan \textit{special case} dari VAR dengan \textit{restrictions} untuk hubungan jangka panjang \autocite{engle1987cointegration}. Model VECM dapat dituliskan sebagai:
...
Dengan $\Pi = \alpha \beta'$ merepresentasikan hubungan kointegrasi jangka panjang \autocite{johansen1995likelihood}.

\subsection{Model \textit{Machine Learning}}
Keterbatasan model ekonometrika tradisional dalam menangkap pola non-linear dapat diatasi oleh algoritma ML. Dalam penelitian ini, model yang digunakan fokus pada pendekatan \textit{Ensemble Learning}. \textit{Ensemble Learning} adalah paradigma ML yang menggabungkan prediksi dari beberapa model dasar (\textit{base learners}) untuk menghasilkan model prediktif yang lebih optimal \autocite{hastie2009elements}. Dua teknik utama dalam \textit{Ensemble Learning} yang digunakan adalah \textit{Bootstrap Aggregating} (Bagging) dan \textit{Boosting}. Bagging bertujuan mengurangi varians dengan melatih model secara paralel pada subset data acak, sedangkan \textit{Boosting} bertujuan mengurangi bias dengan melatih model secara sekuensial untuk memperbaiki kesalahan model sebelumnya.

\subsubsection{Random Forest Regressor}
\textit{Random Forest}, yang diperkenalkan oleh Breiman, adalah implementasi algoritma Bagging yang menggunakan pohon keputusan (\textit{decision trees}) sebagai model dasar \autocite{breiman2001random}. Algoritma ini bekerja dengan langkah-langkah berikut:
\begin{enumerate}
\item \textbf{Bootstrap Sampling}\\
Model akan mengambil $N$ sampel acak dari data latih dengan pengembalian (\textit{replacement}).
\item \textbf{Konstruksi Pohon}\\
Model membangun pohon keputusan untuk setiap sampel. Pada setiap pemecahan simpul (\textit{node split}), algoritma hanya akan mempertimbangkan subset acak dari fitur, bukan seluruhnya. Hal ini akan memaksa pohon-pohon yang terbentuk menjadi tidak berkorelasi.
\item \textbf{Agregasi}\\
Prediksi akhir pada model ini diperoleh dengan menghitung nilai rata-rata output dari seluruh pohon individu.
\end{enumerate}
Mekanisme ini membuat \textit{Random Forest} sangat tahan terhadap \textit{noise} dan \textit{outlier} dalam data keuangan karena kesalahan acak pada satu pohon akan saling menghilangkan saat dirata-ratakan dengan pohon lain. Hal ini meminimalisir risiko \textit{overfitting} pada model dengan signifikan.
...
Model \textit{Deep Learning} berbasis jaringan saraf tiruan (\textit{Neural Networks}) digunakan untuk menangkap struktur dependensi jangka panjang (\textit{long-term dependencies}) yang sering kali luput dari model \textit{Machine Learning} konvensional. Sezer dkk. mencatat bahwa dalam domain keuangan, model berbasis LSTM menjadi arsitektur dominan karena keunggulannya dalam memproses data sekuensial \autocite{sezer2020financial}.

\subsubsection{Long Short-Term Memory (LSTM)}
LSTM adalah varian khusus dari \textit{Recurrent Neural Networks} (RNN) yang dirancang oleh Hochreiter dan Schmidhuber untuk mengatasi masalah \textit{vanishing gradient}, yaitu kondisi ketika RNN gagal mempelajari pola dari data masa lalu yang jauh karena gradien kesalahan menyusut mendekati nol selama proses pelatihan (\textit{backpropagation}) \autocite{hochreiter1997long}. Solusi LSTM terletak pada struktur sel memori (\textit{memory cell}) yang dilindungi oleh tiga gerbang logika (\textit{gates}) untuk mengatur aliran informasi:

\begin{enumerate}
\item \textbf{Forget Gate}\\
Model memutuskan informasi mana dari memori masa lalu yang sudah tidak relevan dan harus dibuang.
\begin{equation}
...
Model akan menentukan informasi baru apa dari input saat ini yang penting untuk disimpan ke dalam memori.
\begin{equation}
...
Model akan mengatur informasi apa dari memori internal yang akan diteruskan sebagai output ke langkah waktu berikutnya.
\begin{equation}
...
Mekanisme ini memungkinkan LSTM untuk mempertahankan informasi penting (seperti dampak kebijakan moneter tahun lalu) dalam jangka waktu panjang dan secara selektif melupakan gangguan jangka pendek (\textit{noise}), menjadikannya sangat efektif untuk data ekonomi yang memiliki \textit{lag effect}.

\subsubsection{Bidirectional Long Short-Term Memory (Bi-LSTM)}
Bi-LSTM merupakan pengembangan arsitektur LSTM yang diperkenalkan oleh Schuster dan Paliwal \autocite{schuster1997bidirectional}. Jika LSTM standar hanya memproses informasi secara maju (\textit{forward}) dari masa lalu ke masa kini ($t-1 \rightarrow t$), Bi-LSTM memproses data dalam dua arah sekaligus menggunakan dua lapisan LSTM terpisah:
\begin{enumerate}
\item \textbf{Forward Layer}: Model memproses urutan data dari awal ke akhir untuk menangkap konteks masa lalu.
\item \textbf{Backward Layer}: Model akan memproses urutan data dari akhir ke awal untuk menangkap konteks masa depan dalam data latih.
\end{enumerate}
Output akhir pada setiap langkah waktu ($y_t$) merupakan gabungan dari kedua lapisan tersebut yang biasanya melalui konkatenasi atau penjumlahan:

\begin{equation}
y_t = W_{\vec{h}} \vec{h}_t + W_{\overleftarrow{h}} \overleftarrow{h}_t + b_y
\label{eq:bilstm}
\end{equation}

Output dari kedua lapisan ini kemudian digabungkan untuk menghasilkan representasi fitur yang lebih baik. Dalam konteks prediksi \textit{deret waktu}, pendekatan ini memungkinkan model untuk memahami struktur data secara lebih holistik, mendeteksi pola kausalitas dan korelasi yang mungkin terlewat jika hanya dilihat dari satu arah sekuensial \autocite{schuster1997bidirectional}.
...
di-permutasi atau diacak \autocite{breiman2001random}. 
...
penting \autocite{molnar2022interpretable}.
...
bias \autocite{molnar2022interpretable}.
